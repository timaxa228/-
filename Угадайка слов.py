...
def play(word):
    ...
    while True:
        vvod = input('Введите букву или слово: ').upper()
        if vvod.isalpha():
            if vvod in guessed_letters:
                ...
                for i in range(len(word)):
                    if word[i] == vvod:
                        word_completion[i] = vvod
                    else:
                        continue
                word_str = ''.join(word_completion)
                print(word_str)
                if word_str == word:
...
while True:
    play(get_word())
    if input('Хотите сыграть ещё раз? ').lower() == 'да':
        continue
    break
